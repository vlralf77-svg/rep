...
type TabValue = Sport | 'toto' | 'proto';
...
export default function DashboardPage() {
  const [tab, setTab] = useState<TabValue>('football');
  const [league, setLeague] = useState('PL');
  const [status, setStatus] = useState('');

  const sport: Sport = (tab === 'toto' || tab === 'proto') ? 'football' : tab;
  const isBetman = tab === 'toto' || tab === 'proto';
  ...
  return (
    <>
      <AppBar position="sticky" elevation={0} sx={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 2, minHeight: 40 }} variant="scrollable" scrollButtons="auto">
          ...
          <Tab value="toto" label="토토 배당" icon={<CasinoIcon fontSize="small" />} iconPosition="start" sx={{ minHeight: 40, fontSize: 13 }} />
          <Tab value="proto" label="프로토 배당" icon={<CasinoIcon fontSize="small" />} iconPosition="start" sx={{ minHeight: 40, fontSize: 13 }} />
        </Tabs>
      </AppBar>

      <Container maxWidth="md" sx={{ py: 3 }}>
        {isBetman ? (
          <BetmanGames type={tab as 'toto' | 'proto'} />
        ) : (
          <>
